...
public class Worker extends Thread {
	...
	/**
	 * keep reference to the file system structure
	 */
	 ...
	 */
	public Worker(Socket client, Database database, FileSystem fileSystem, int dataPort) {
		super();
	...
	public void run() {
		...
		try {
			...
			/* notify client that the connection was established successfully */
			sendMsgToClient("220");
	...
	private void handleNlst(String path) {
		if (dataConnection == null || dataConnection.isClosed()) {
			...
			List<String> dirContent = this.fileSystem.ls(path);
			...
			if (dirContent == null) {
				...
				for (int i = 0; i < dirContent.size(); ++i) {
					sendDataMsgToClient(dirContent.get(i));
				}
	...
	private void handleMkd(String path) {
		...
		if (name != null && name.matches("^[a-zA-Z0-9]+$")) {
			if (this.fileSystem.mkdir(path.substring(0, lastIndex), name) == false) {
	...
	private void handleRetr(String path) {
		...
		if (transferMode == transferType.BINARY) {
			...
			if (this.fileSystem.download(path, FileType.BINARY,
			...
			if (this.fileSystem.download(path, FileType.ASCII,
	...
	private void handleDele(String path) {
		...
		if (this.fileSystem.rm(path) == false) {
	...
	private void _handleFile(String path, boolean append) {
		...
		/* Binary mode */
		if (transferMode == transferType.BINARY) {
			result = this.fileSystem.upload(path.substring(0, lastIndex), name, FileType.BINARY,
					new BufferedInputStream(stream), append);
		} else {
			/* ASCII mode */
			result = this.fileSystem.upload(path.substring(0, lastIndex), name, FileType.ASCII,
